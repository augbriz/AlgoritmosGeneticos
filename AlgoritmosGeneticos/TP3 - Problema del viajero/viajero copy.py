...
while True:
    ...
    metodo_seleccion = input("Ingrese 1 o 2: ")
    if metodo_seleccion in ['1', '2']:
        break
    else:
...
def seleccion_por_ruleta(poblacion):
    global distancias
    fitnesses = [1 / calcular_distancia(ind, distancias) for ind in poblacion]
    total = sum(fitnesses)
...
def algoritmo_genetico():
    global distancias, tam_poblacion, generaciones, tasa_mutacion, n_ciudades, metodo_seleccion, elitismo
    ...
    for gen in range(generaciones):
        ...
        # Elitismo solo si se usa ruleta y está activado
        if metodo_seleccion == "1" and elitismo:
            nueva_poblacion.append(mejor.copy())

        while len(nueva_poblacion) < tam_poblacion:
            if metodo_seleccion == "2":
                padre1 = seleccion_por_torneo(poblacion)
                padre2 = seleccion_por_torneo(poblacion)
            elif metodo_seleccion == "1":
                padre1 = seleccion_por_ruleta(poblacion)
                padre2 = seleccion_por_ruleta(poblacion)
            else:
                raise ValueError("Método de selección no reconocido. Usa '1' (ruleta) o '2' (torneo).")
